...
banco = sqlite3.connect("achados_e_perdidos.db")
...
cursor.execute('''
                CREATE TABLE IF NOT EXISTS usuarios(
                    ...
                    tipo_usuario TEXT NOT NULL CHECK(
                        tipo_usuario IN('aluno', 'funcionario')), 
                    ra TEXT, curso TEXT, 
                    periodo TEXT NOT NULL CHECK(
                        periodo IN('Integral', 'Noturno')), 
...
cursor.execute(''' 
                CREATE TABLE IF NOT EXISTS itens(
                    ...
                    tipo_item TEXT NOT NULL CHECK(
                        tipo_item IN ('Perdido', 'Achado')), 
                    status TEXT NOT NULL DEFAULT 'Ativo' CHECK(
                        status IN ('Ativo', 'Reivindicado', 'Entregue', 'Arquivado')), 
